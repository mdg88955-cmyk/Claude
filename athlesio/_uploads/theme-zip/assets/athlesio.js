...
(function () {
  ...
  if (form) {
    ...
    var submit = form.querySelector('[data-add]');
    ...
    form.querySelectorAll('[data-variant]').forEach(function (row) {
      row.addEventListener('click', function () {
        ...
        if (idInput) idInput.value = row.getAttribute('data-variant-id');
        ...
        if (submit) {
          ...
          submit.textContent = avail ? submit.getAttribute('data-label') : 'Sold out';
        }
      });
    });

    /* Sticky bar button submits the real form */
    var sAdd = document.querySelector('[data-sticky-add]');
    if (sAdd) {
